$(document).ready(function() {
    ...
    //var hotel_id;
    //console.log(hotel_id+'hi');
    var base_url = "http://giovannis37.sg-host.com";
    var evens;
    $.ajax({
        url:base_url+'/get-events/'+hotel_id,
        type:'get',
        dataType:'json',
        ...
        var calendar = $('#calendar').fullCalendar({
            ...
            eventClick: function(info) {
                ...
                //me
                var eventDate=info.eventDate;
                //var reserveDate=info.reserveDate;
                var numberOfPeople=info.numberOfPeople;
                var numberOfChildren=info.numberOfChildren;
                var hoursFrom=info.hoursFrom;
                var hoursTo=info.hoursTo;
                ...
                //var rd = $('#reserve-date');
                //rd.val(reserveDate);
                var np = $('#number-of-people');
                np.val(numberOfPeople);
                var nc = $('#number-of-children');
                nc.val(numberOfChildren);
                var hf = $('#hours-from');
                hf.val(hoursFrom);
                var ht = $('#hours-to');
                ht.val(hoursTo);
                ...
                var startDate = flatpickr(document.getElementById('start-date'), {
                    enableTime: true,
                    dateFormat: "Y-m-d",
                    defaultDate: info.start.format("YYYY-MM-DD"),
                });
    ...
    $("#add-e").off('click').on('click', function(event) {
        ...
        var hoursFrom = $("#hours-from").val();
        var hoursTo = $("#hours-to").val();
        var customer = $("#customer").val();
        var service = $("#service").val();

        var customerText=$("#customer option:selected").text();
        ...
        var concatenateStartDateTime = startDate;
        ...
        var myEvent = {
          ...
          title:customerText+'-'+serviceText,
        ...
        var evens;
        $.ajax({
            url:base_url+'/submit-event/'+concatenateStartDateTime+'/'+eventDate+'/'+numberOfChildren+'/'+numberOfPeople+'/'+hoursFrom+'/'+hoursTo+'/'+service+'/'+customer,
            type:'get',
            dataType:'json',
            success:function (response) {
